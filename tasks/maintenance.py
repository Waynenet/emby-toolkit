...
def task_import_database(processor, file_content: str, tables_to_import: List[str], import_strategy: str):
    ...
    # ★★★ 共享白名单 ★★★
    SHARABLE_TABLES = {'person_metadata', 'translation_cache', 'media_metadata'}
    
    # ★★★ 为新表添加中文名 ★★★
    TABLE_TRANSLATIONS = {
        'person_metadata': '人物元数据',
        'translation_cache': '翻译缓存',
        'douban_api_cache': '豆瓣数据缓存',
        ...
        with connection.get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                for table_name in sorted_tables_to_import:
                    ...
                    # ==========================================
                    # ★★★ 新增：全局清理废弃字段 ★★★
                    # 无论后续是 share 还是 overwrite，都提前移除已废弃列，防止数据库报错
                    # ==========================================
                    if table_name.lower() == 'media_metadata':
                        for row in table_data:
                            row.pop('file_sha1_json', None)
                            row.pop('file_pickcode_json', None)
                    # ==========================================
