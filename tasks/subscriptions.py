...
def _hdhive_lock_key(tmdb_id, media_type):
    if str(media_type or '').strip().lower() != 'tv':
        return ''
    return str(tmdb_id or '').strip()


def _get_hdhive_locked_link(tmdb_id, media_type):
    key = _hdhive_lock_key(tmdb_id, media_type)
    if not key:
        return None
    ...
    item = locked_links.get(key) if isinstance(locked_links, dict) else None
...
def _save_hdhive_locked_link(tmdb_id, media_type, resource, title=''):
    ...
    logger.info("  ➜ [影巢锁定] 已自动锁定剧集 TMDB %s 的影巢链接: %s", key, slug)
    return item
...
def _try_download_from_hdhive_first(tmdb_id, media_type, title, item_label="媒体", target_season=None, require_complete=False):
    ...
    try:
        ...
        locked_link = _get_hdhive_locked_link(tmdb_id, media_type)
        if locked_link:
            locked_slug = locked_link.get('slug')
            logger.info(
                "  ➜ [影巢锁定] 《%s》已锁定影巢链接，本次追更复用 slug=%s",
                title,
                locked_slug,
            )
            ...
            if success:
                logger.info("  ➜ 影巢锁定链接处理成功！已跳过 MoviePilot 订阅。")
                return True
            logger.warning("  ➜ 影巢锁定链接处理失败，将重新检索候选资源。")
        ...
        if success:
            _save_hdhive_locked_link(tmdb_id, media_type, target_resource, title=title)
