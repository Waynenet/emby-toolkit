...
def update_subscription_episode_priority(
    tmdb_id: str,
    season: int,
    ...
    *,
    ...
    preserve_best_version_full: bool = False,
) -> bool:
    ...
    priority = existing.get("episode_priority")
    ...
    if priority_updates:
        ...
        for key, value in priority_updates.items():
            if priority.get(key) != value:
                priority[key] = value
                changed = True
        target_current_priority = None
        if preserve_best_version_full and baseline_priority is not None:
            target_current_priority = int(baseline_priority)
            if _safe_int_value(existing.get("current_priority"), -1) != target_current_priority:
                changed = True
        if not changed:
            return True
        ...
        if target_current_priority is not None:
            payload["current_priority"] = target_current_priority
        if not preserve_best_version_full:
    ...
    if baseline_priority is None:
        ...
        for value in priority.values():
            try:
                ...
                pass
        baseline_priority = max(values) if values else None
    if baseline_priority is None:
        return False

    baseline_priority = int(baseline_priority)
    washing_priority = max(baseline_priority - 1, 0)

    changed = False
    for ep in washing:
        key = str(ep)
        if priority.get(key) != washing_priority:
            priority[key] = washing_priority
            changed = True
    for ep in completed:
        key = str(ep)
        if priority.get(key) != baseline_priority:
            priority[key] = baseline_priority
            changed = True
    if not changed:
        return True
    ...
    payload["episode_priority"] = priority
    payload["best_version"] = 1
    if preserve_best_version_full:
        payload["current_priority"] = int(baseline_priority)
    if not preserve_best_version_full:
        payload.pop("best_version_full", None)
    payload["season"] = int(season)
    payload["tmdbid"] = int(tmdb_id)
