...
- 网盘文件整理：整理识别待整理目录中媒体项归类到分类目录并同步生成STRM
- 全量生成STRM：遍历所有分类目录在本地媒体库生成STRM，已存在且内容无变化的会跳过
- 同步媒体信息：补齐媒体指纹，并把数据库缓存注入对应 Emby Item
- 重建媒体信息：从 RAW 重建格式化缓存，并注入所有在库 Emby Item
