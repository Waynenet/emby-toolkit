...
- 媒体信息约定：`p115_mediainfo_cache.mediainfo_json` 是格式化数据源，通过 `plugins/emby-mediainfo-bridge` 直接注入 Emby；不生成或读取 `*-mediainfo.json`，不使用 Emby 远程探测替代 ETK 格式化结果。Emby 手动或任务刷新后，插件通过 `/api/p115/mediainfo/...` 自动回补媒体流和章节；智能追剧递归刷新 Series 后，ETK 收尾阶段会批量回补全部 Episode 实际版本。
