...
4. 注入成功的 Item ID 进入核心处理器，完成人物翻译和处理后先提交完整 `media_metadata`，再替换刷新 Emby 元数据和图片；插件从数据库恢复完整人物表及四件套图片，季海报按 Season 返回。普通刷新复用持久化图片快照；Emby 刷新请求带 `ReplaceAllImages=true` 时，插件 Harmony 前置钩子会在 Emby 开始删除和抓取图片前同步调用 `/api/emby/metadata/images/refresh`，ETK 按当前“简体中文优先/原语言优先”重新查询 TMDb 并覆盖数据库图片路径。同一插件的图片搜索 Provider 通过 `/api/emby/metadata/images/search` 每次实时返回 TMDb 全量图片候选、语言、分辨率、评分和票数，候选按 ETK 语言偏好排序但不过滤其他语言。截图始终只在 ETK 临时目录生成并上传 Emby，不在媒体目录落地。可选的“持久化元数据图片”会把最终采用的电影/剧集四件套、季海报、分集正式剧照和合集图片按 SHA256 保存到可配置仓库，数据库记录来源地址、相对路径、MIME 和大小；Provider 通过现有 `/api/image_proxy` 返回本地镜像。Episode 无正式剧照时用独立 `screenshot_hash` 保存当前兜底截图：没有正式剧照就无限期保留，一旦正式剧照写库并成功接管 Emby 图片就删除截图索引和物理文件，重截时删除旧版，只保留当前截图；开启仓库前已存在的 Emby 旧截图会在 Episode 后续刷新时按持久化 ItemID 读取当前 Primary 并自动迁移。家庭视频没有稳定 TMDb 身份，暂不纳入图片仓库。“补齐视频截图”任务沿用旧 `restore-nfo-and-images` key，扫描 Emby 缺少主图的 Movie/Episode/Video STRM 并补图，不再还原 NFO 或媒体目录图片。
