...
- `p115_rename.py`：命名规则。
- `p115_upload_monitor.py`：本地目录到 115 的增量分片上传监控，保存续传任务状态并支持上传成功后删除源文件。
...
4. 注入成功的 Item ID 进入核心处理器，完成人物翻译和处理后先提交完整 `media_metadata`，再替换刷新 Emby 元数据和图片；插件从数据库恢复完整人物表及四件套图片，季海报按 Season 返回。智能追剧的缺图分集使用 ETK 临时目录生成截图并上传到 Emby `Primary` 缓存，上传后立即删除临时文件；TMDb 后续出图时先写入 `media_metadata.poster_path`，再通过 Emby `RemoteImages/Download` 显式下载 ETK 正式剧照覆盖截图缓存。常规刷新和重新整理也会用该显式下载接口回补缺失的四件套和季海报，不依赖 Emby `ReplaceAllImages` 自动抓取。ETK STRM 媒体目录不持久化分集截图。
