...
1. 实时监控识别新增 STRM 或实体媒体，并准备简版 NFO；实体文件的 ffprobe 结果保留在内存，STRM 的格式化媒体信息来自 `p115_mediainfo_cache`。
2. ETK 主动通知 Emby 扫描。对新 STRM，桥接插件从 `ItemAdded/ItemUpdated` 事件取得 ItemID，媒体信息注入成功后调用 ETK `item-ready` 接口；ETK 校验 pick code/SHA1 与 Item 归属后进入后续处理。
3. ETK 为 STRM 插件上报保留 8 秒优先窗口，超时后才按媒体路径轮询 Item ID 兜底；物理媒体仍直接轮询。ETK 通过独立的 [ETK MediaInfo Bridge](https://github.com/hbq0405/etk-mediainfo-bridge) 插件将格式化媒体流直接持久化到对应 Emby Item；不生成或读取 `*-mediainfo.json`，也不触发 Emby 远程探测。
4. 注入成功的 Item ID 进入核心处理器，完成元数据补全、翻译、中文化、数据库写入和 Emby 回写。
...
媒体信息桥接插件的接口是 `POST /Items/{Id}/ETKMediaInfo`。每个实际版本必须使用自己的 Emby Item ID；插件覆盖内嵌流时保留 Emby 已识别的外挂流，并同步章节、时长、大小、封装、码率和分辨率。插件同时监听 Emby Item 刷新事件，刷新稳定后根据 STRM 中的 pick code 或虚拟播放 SHA1 调用 `/api/p115/mediainfo/...` 重新注入；事件按 Item 合并并限制为 4 路并发。
