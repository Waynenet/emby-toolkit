...
共享片头继续使用中心端独立片头索引，不进入共享 RAW。桥接插件处理 Emby Item 更新时会在媒体信息恢复请求中携带 ItemId；ETK 使用自身 Emby 凭据读取 `Chapters`，并通过 `MediaSources.Path` 校验 Item 与 pick code/SHA1 对应关系。检测到完整 `IntroStart`/`IntroEnd` 后，ETK 只合并到 `p115_mediainfo_cache.mediainfo_json.Chapters`，再异步上传中心；`raw_ffprobe_json` 保持原始探测数据。
