...
1. 实时监控识别新增 STRM 或实体媒体。ETK STRM 在通知 Emby 前把除人物表外的完整 TMDb 元数据写入 `media_metadata`，并保存海报、背景、Logo、横版缩略图和季海报路径；实体媒体继续使用 NFO 兜底。实体文件的 ffprobe 结果保留在内存，STRM 的格式化媒体流来自 `p115_mediainfo_cache`。
2. ETK 主动通知 Emby 扫描。桥接插件的 Movie/Series/Season/Episode Provider 根据 STRM pick code/SHA1 请求 ETK 元数据接口，首次扫描直接返回确定的 TMDb 身份，不依赖目录 TMDb 尾巴或 NFO。Season 元数据响应不得携带集号，Episode 才同时返回季号和集号；插件按项目类型分别设置 `IndexNumber`/`ParentIndexNumber`，避免目录中任意 STRM 的集号污染季索引。插件从 `ItemAdded/ItemUpdated` 事件取得 ItemID，媒体信息注入成功后调用 ETK `item-ready` 接口；ETK 校验 pick code/SHA1 与 Item 归属后进入后续处理。剧集 ItemID 按 SeriesId 等待 3 秒聚合后一次提交，同一 ItemID 在插件回调与路径兜底之间只允许分派一次，待提交队列也会合并同剧的 `new_episode_ids`。
3. ETK 为 STRM 插件上报保留 8 秒优先窗口，超时后才按媒体路径轮询 Item ID 兜底；物理媒体仍直接轮询。ETK 通过独立的 [ETK MediaInfo Bridge](https://github.com/hbq0405/etk-mediainfo-bridge) 插件将格式化媒体流直接持久化到对应 Emby Item；不生成或读取 `*-mediainfo.json`，ETK STRM 也不再生成简版或完整 NFO。
4. 注入成功的 Item ID 进入核心处理器，完成人物翻译和处理后先提交完整 `media_metadata`，再替换刷新 Emby 元数据和图片；插件从数据库恢复完整人物表及四件套图片，季海报按 Season 返回。智能追剧和家庭视频的缺图视频使用 ETK 临时目录生成截图并上传到 Emby `Primary` 缓存，上传后立即删除临时文件；“补齐视频截图”任务沿用旧 `restore-nfo-and-images` key，扫描 Emby 缺少主图的 Movie/Episode/Video STRM 并补图，不再还原 NFO 或媒体目录图片。TMDb 后续出图时先写入 `media_metadata.poster_path`，再通过 Emby `RemoteImages/Download` 显式下载 ETK 正式剧照覆盖截图缓存。常规刷新和重新整理也会用该显式下载接口回补缺失的四件套和季海报，不依赖 Emby `ReplaceAllImages` 自动抓取。ETK STRM 媒体目录不持久化分集截图。
