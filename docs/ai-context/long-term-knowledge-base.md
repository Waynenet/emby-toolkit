...
2. ETK 主动通知 Emby 扫描。对新 STRM，桥接插件从 `ItemAdded/ItemUpdated` 事件取得 ItemID，媒体信息注入成功后调用 ETK `item-ready` 接口；ETK 校验 pick code/SHA1 与 Item 归属后进入后续处理。剧集 ItemID 按 SeriesId 等待 3 秒聚合后一次提交，同一 ItemID 在插件回调与路径兜底之间只允许分派一次，待提交队列也会合并同剧的 `new_episode_ids`。
