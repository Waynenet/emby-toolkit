...
插件更新不再由 Emby 直连 GitHub。计划任务从持久化的 ETK 服务地址请求固定端点 `/api/emby/plugin-update`；ETK 使用全局 `network_http_proxy` 配置访问官方 Release，并以流式响应转发 `ETKMediaInfoBridge.dll`。该端点不是通用 URL 代理，插件仍会在本地校验程序集名称和版本后才替换 DLL；无法确定 ETK 地址时直接失败，不回退 GitHub 直连。
